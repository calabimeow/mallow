...
{
    return window_data->canvas->width;
}

int get_screen_height()
{
    return window_data->canvas->height;
}

vec2 get_screen_size()
{
    return (vec2){window_data->canvas->width, window_data->canvas->height};
}

vec2 get_screen_center()
{
    return (vec2){(float)window_data->canvas->width / 2, (float)window_data->canvas->height / 2};
}
...
{
    vec2 screen_pos = get_world_to_screen((vec2){x, y}, current_cam);
    qtee_circle(*window_data->canvas, screen_pos.x, screen_pos.y, r, color);
}

void draw_circle_outline(int x, int y, size_t r, int thick, uint32_t color)
{
    vec2 screen_pos = get_world_to_screen((vec2){x, y}, current_cam);
    qtee_circle_outline(*window_data->canvas, screen_pos.x, screen_pos.y, r, thick, color);
}

void draw_text(const char *text, int x, int y, size_t size, uint32_t color)
{
    vec2 screen_pos = get_world_to_screen((vec2){x, y}, current_cam);
    qtee_text(*window_data->canvas, text, screen_pos.x, screen_pos.y,
            qtee_default_font, size, color);
}
...
// files
void write_ppm(const char *path, texture *texture)
{
    qtee_canvas canvas = qtee_create_canvas(texture->pixels, texture->width, texture->height);
    qtee_write_ppm(canvas, path);
}

texture load_texture(const char *path)
{
    qtee_canvas canvas = qtee_load_texture(path);
